...
exports.getReportsSummary = exports.getTopProductsReport = exports.getSalesChartData = exports.getDashboardMetrics = void 0;
...
const getDashboardMetrics = async (req, res) => {
    try {
        ...
        const todayInvoices = await Invoice_1.Invoice.find({
        ...
        const allInvoices = await Invoice_1.Invoice.find({ organizationId: orgId, isRefunded: false });
        const totalRevenue = allInvoices.reduce((sum, inv) => sum + inv.grandTotal, 0);
        // Low Stock Products Count
        const products = await Product_1.Product.find({ organizationId: orgId, activeStatus: true, isService: false });
        const lowStockCount = products.filter(p => p.currentStock <= p.minimumStock).length;
        // Recent 5 Orders
        const recentOrders = await Order_1.Order.find({ organizationId: orgId }).sort({ createdAt: -1 }).limit(5);
        ...
        return (0, response_1.sendError)(res, err.message || 'Failed to fetch dashboard metrics', 500);
    }
};
...
const getSalesChartData = async (req, res) => {
    try {
        ...
        return (0, response_1.sendSuccess)(res, chartData, 'Sales chart data generated');
    }
    catch (err) {
        return (0, response_1.sendError)(res, err.message || 'Failed to fetch sales chart', 500);
    }
};
...
const getTopProductsReport = async (req, res) => {
    try {
        ...
        const topProducts = Object.keys(productStats)
            ...
            .slice(0, 10);
        return (0, response_1.sendSuccess)(res, topProducts, 'Top products report loaded');
    }
    catch (err) {
        return (0, response_1.sendError)(res, err.message || 'Failed to fetch top products', 500);
    }
};
exports.getTopProductsReport = getTopProductsReport;
const getReportsSummary = async (req, res) => {
    try {
        ...
        // Completed orders
        const completedOrders = await Order_1.Order.find({ organizationId: orgId, status: 'COMPLETED' });
        ...
        // Sales by Category
        const products = await Product_1.Product.find({ organizationId: orgId });
        const categories = await Category_1.Category.find({ organizationId: orgId });
        ...
        return (0, response_1.sendError)(res, err.message || 'Failed to fetch reports summary', 500);
    }
};
